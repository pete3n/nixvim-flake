      ...
      luaConfig.pre = # lua
        ''
          vim.api.nvim_create_user_command("OpenNixvimConfig", function()
            local viminit = vim.env.VIMINIT or ""
            local init_path = viminit:match("dofile%('([^']+)'%)")
            if init_path and vim.fn.filereadable(init_path) == 1 then
              vim.cmd.edit(init_path)
            else
              vim.notify("Could not find Nixvim init.lua in VIMINIT", vim.log.levels.ERROR)
            end
          end, {})
        '';
      settings = {
